...
/*
 * MediaTek PCIe Hotplug Driver for NVIDIA CX7 NIC
 *
...
static int cx7_hp_map_mmio_resources(struct cx7_hp_dev *dev)
{
     ...
     for (i = 0; i < parsed.count; i++) {
         ...
         case 0:
             if (dev->pd->port_nums >= 1) {
                 ...
                 if (!base) {
                     dev_err(&pdev->dev, "Failed to map MAC Port 0 region (0x%08x)\n", addr);
                     return -ENOMEM;
                 }
                 dev->mmio.mac_port_base[0] = base;
                 mapped_count++;
                 dev_info(&pdev->dev, "Mapped MAC Port 0: 0x%08x (size 0x%x) -> %p\n",
                      addr, size, base);
             }
             break;
         case 1:
             if (dev->pd->port_nums >= 2) {
                 base = devm_ioremap(&pdev->dev, addr, size);
                 if (!base) {
                     dev_err(&pdev->dev, "Failed to map MAC Port 1 region (0x%08x)\n", addr);
                     return -ENOMEM;
                 }
                 dev->mmio.mac_port_base[1] = base;
                 mapped_count++;
                 dev_info(&pdev->dev, "Mapped MAC Port 1: 0x%08x (size 0x%x) -> %p\n",
                      addr, size, base);
             }
             break;
         case 2:
         ...
         case 4:
             ...
             mapped_count++;
             dev_info(&pdev->dev, "Mapped CKM: 0x%08x (size 0x%x) -> %p\n",
                  addr, size, base);
             break;
         default:
             dev_warn(&pdev->dev, "Unexpected MMIO region at 0x%08x (size 0x%x), skipping\n",
                 addr, size);
             break;
         }
     }
    ...
    if (!dev->mmio.top_base || !dev->mmio.protect_base || !dev->mmio.ckm_base ||
        ...
        dev_err(&pdev->dev, "Required MMIO regions not mapped from ACPI _CRS (mapped %d)\n",
             mapped_count);
 ...
 */
 ...
 get_port_root_port(struct cx7_hp_dev *hp_dev, int port_idx)
 {
                                  ...
                                  struct cx7_hp_plat_data *pd)
{
    ...
    if (pd->port_nums == 0 || pd->port_nums >= HP_PORT_MAX) {
        dev_err(&pdev->dev, "Invalid port count from _DSD: %d (must be 1-%d)\n",
                                     ...
                                     struct cx7_hp_dev *hp_dev)
{
    ...
    if (walk_ctx.count < CX7_HP_MIN_GPIO_COUNT) {
        dev_err(&pdev->dev, "Insufficient GPIOs from ACPI: required at least %d, got %d\n",
    ...
    if (walk_ctx.count == 0 || walk_ctx.gpios[0].resource_source[0] == '\0') {
        dev_err(&pdev->dev, "No resource_source in ACPI GPIO resources\n");
        return -ENODEV;
    }
    ...
    if (ACPI_FAILURE(status)) {
        dev_err(&pdev->dev, "Failed to get ACPI handle for GPIO controller %s\n",
                walk_ctx.gpios[0].resource_source);
        return -ENODEV;
    }
    ...
    if (!gpio_adev) {
        dev_err(&pdev->dev, "Failed to get ACPI device for GPIO controller %s\n",
                walk_ctx.gpios[0].resource_source);
        return -ENODEV;
    }
    ...
    for (i = 0; i < hp_dev->gpio_count; i++) {
        ...
        dev_info(&pdev->dev, "Using ACPI GPIO[%d]: pin %u (chip-relative) -> global GPIO %u\n",
